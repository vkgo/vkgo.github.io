...
\newcommand{\me}[1]{{\bfseries\color{ink}#1}}            % emphasize without adding color
\newcommand{\venue}[2]{{\bfseries\color{primary}#1}\,{\color{muted}\footnotesize #2}}
...
\pub
  ...
  {We address the gap between visually strong reference dances and motions that remain stable after humanoid
   control. ChoreoPlan combines beat-snapped hybrid phrase planning with an Embodied Selector trained from
   offline controller rollouts to favor executable, semantically aligned candidates. Across AIST++ and FineDance
   tracks in IsaacGym and MuJoCo, it improves rollout success, tracking, and rhythmic and semantic retention,
   with transfer to a Unitree G1.}
  {}
